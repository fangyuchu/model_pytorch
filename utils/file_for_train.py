...
if dataset == 'cifar10':
    ...
    if net_type =='resnet56':
        exp_name='resnet56_cifar100_predicted_mask_and_variable_shortcut_net_mask_newinner_4'
        description=exp_name+'  '+'专门训练mask,没有warmup，训练20epoch'

        total_flop=126556516
        prune_ratio=0.70
        for prune_ratio in [0.75,0.8,0.83,0.85,0.87,0.9,0.93,0.95,0.98,0.5,0.6,0.65,0.7]:
            ...
            net=resnet_cifar.resnet56(num_classes=100).to(device)
            ...
            checkpoint = torch.load(os.path.join(conf.root_path, 'masked_net','resnet56_cifar100', str(i) + '.tar'),map_location='cpu')
            net.load_state_dict(checkpoint['state_dict'])
            ...
            net.prune_net()
            net.current_epoch = net.mask_training_stop_epoch + 1
            learning_rate_decay_epoch = [60,120,160]#[1*i for i in [80,120]]##
        ...
        exp_name='vgg16bn_cifar100_predicted_mask_and_variable_shortcut_net_mask_newinner_5'
        description=exp_name+'  '+'专门训练mask,没有warmup，训练20epoch'
    ...
    if net_type == 'resnet50':
        ...
        learning_rate_decay_epoch = [1 * i for i in [30, 60,90]]
        num_epochs = 100
